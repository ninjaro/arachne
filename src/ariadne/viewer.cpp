...
nlohmann::ordered_json viewer_builder::build_site(
    ...
    constexpr std::array template_files { "index.html", "app.js",
                                          "styles.css" };
    std::map<std::string, std::string, std::less<>> content;
    for (const auto* filename : template_files) {
        content.emplace(filename, read_file(template_root / filename));
    }
    content.emplace("data/projection.json", projection.dump(2) + "\n");
    ...
    content.emplace("build-info.json", build_info.dump(2) + "\n");
    ...
    nlohmann::ordered_json manifest {
        ...
        { "product_snapshot_id", projection.at("product_snapshot_id") },
        { "candidate_snapshot_id", projection.at("candidate_snapshot_id") },
        { "viewer_version", "ariadne-static-viewer-1.0.0" },
