...
graph_input parse_graph(const nlohmann::json& document) {
    ...
    require_stable_id(
        result.source_snapshot_id, "external_graph.source_snapshot.snapshot_id"
    );
    if (result.source_sha256.size() != 64
        ...
        throw std::invalid_argument("source_sha256 must contain 64 hex characters");
    }
    if (!document.contains("works") || !document.at("works").is_array()
        || !document.contains("agents") || !document.at("agents").is_array()
        || !document.contains("edges") || !document.at("edges").is_array()) {
        throw std::invalid_argument(
    ...
    for (const auto& value : document.at("works")) {
        ...
        if (!value.contains("covered") || !value.at("covered").is_boolean()) {
    ...
    for (const auto& value : document.at("agents")) {
        ...
        if (!value.contains("profile") || !value.at("profile").is_object()) {
    ...
    for (auto& agent : result.agents) {
        std::ranges::sort(agent.works, [&](const auto left, const auto right) {
            return id_less(result.works[left].id, result.works[right].id);
        });
    }
    return result;
}
...
void validate_configuration(const candidate_configuration& configuration) {
    ...
    if (configuration.target_size > configuration.pool_size) {
        throw std::invalid_argument("candidate target cannot exceed pool size");
    }
    if (configuration.gray_bonus_basis_points < 0) {
        throw std::invalid_argument("gray bonus must be non-negative");
    }
    if (!std::isfinite(configuration.quality_weight)
        || configuration.quality_weight < 0.0
        || configuration.quality_weight > 1.0) {
        throw std::invalid_argument("quality weight must be between zero and one");
    }
}
...
void collect_profile_features(
    ...
    if (value.is_number_integer() && prefix.find("year") != std::string::npos) {
    ...
    if (value.is_object()) {
        for (auto iterator = value.begin(); iterator != value.end(); ++iterator) {
            const std::string child_prefix = prefix.empty()
                ? iterator.key()
                : prefix + "." + iterator.key();
            collect_profile_features(iterator.value(), child_prefix, output);
        }
    }
}

double weighted_jaccard(
    const feature_vector& left, const feature_vector& right
) {
...
void group_and_select(
    ...
    for (std::size_t index = 0; index < pool.size(); ++index) {
        collect_profile_features(
            graph.agents[pool[index].agent_index].profile, {}, features[index]
        );
    }
        ...
        for (std::size_t candidate = 0; candidate < pool.size(); ++candidate) {
            ...
            const double quality = 1.0
                - static_cast<double>(candidate)
                    / static_cast<double>(std::max<std::size_t>(1, pool.size() - 1));
            const double score = 0.75 * (1.0 - maximum_similarity) + 0.25 * quality;
            if (score > best_score
                || (!(score < best_score)
                    && (!best.has_value()
                        || id_less(
    ...
    for (std::size_t group = 0; group < group_count; ++group) {
        ...
        for (std::size_t index = 0; index < std::min(quota, scores.size()); ++index) {
            keep[scores[index].second] = true;
        }
    }

    // Preserve as much of the first-pass work coverage as possible. A work
    // owned by an excluded pool member may move only to a retained agent that
    // has the same source-graph edge. This does not invent relationships.
    ...
    for (std::size_t index = 0; index < pool.size(); ++index) {
        ...
        retained_works.insert(
            pool[index].claimed_works.begin(), pool[index].claimed_works.end()
        );
    }
        ...
        for (const auto work_index : pool[excluded].claimed_works) {
            ...
            for (std::size_t candidate = 0; candidate < pool.size(); ++candidate) {
                ...
                const auto better = [&]() {
                    ...
                    if (retained_load[candidate] != retained_load[*chosen]) {
            ...
            if (chosen.has_value()) {
                ...
                std::ranges::sort(
                    ...
                    [&](const auto left, const auto right) {
                        return id_less(graph.works[left].id, graph.works[right].id);
                    }
                );
                ++retained_load[*chosen];
...
std::string selection_explanation(const ranked_candidate& candidate) {
    return "Selected by deterministic coverage/grey-node ranking at pool rank "
...
void publish_immutable_file(
    ...
    if (destination.empty() || destination.filename().empty()) {
        throw std::invalid_argument("candidate plan destination must be a file");
    }
    ...
    if (std::filesystem::exists(destination)) {
        if (!std::filesystem::is_regular_file(destination)
            ...
            throw std::runtime_error(
                "candidate plan destination already contains different bytes"
            );
        }
        return;
    }
    ...
    if (std::filesystem::exists(staging)) {
        throw std::runtime_error("candidate plan staging file already exists");
    }
    try {
        {
            std::ofstream output(
            ...
            if (!output) {
                throw std::runtime_error("cannot create candidate plan artifact");
            }
            output.write(
            ...
            output.close();
            if (!output) {
                throw std::runtime_error("cannot finish candidate plan artifact");
            }
        }
...
nlohmann::ordered_json candidate_planner::build(
    ...
    const auto configuration_hash = crypto::sha256(
        arachnespace::contracts::canonical_json(values)
    );
        ...
        for (const auto work_index : candidate.claimed_works) {
            ...
            if (emitted_works.emplace(work_index).second) {
                plan["works"].push_back(
                    { { "work_id", work_id },
                      { "candidate_id", candidate_id },
                      { "external_id", work.id }, { "label", work.label },
                      { "source_snapshot_id", graph.source_snapshot_id },
                      { "attributes",
                        { { "noncanonical", true },
                          { "soft_guidance", true } } } }
    ...
    for (std::size_t group = 0; group < configuration.group_count; ++group) {
        plan["groups"].push_back(
            { { "group_id", "group-" + std::to_string(group + 1) },
              { "label", "Research suggestion group " + std::to_string(group + 1) },
              { "order", group },
    ...
    std::ranges::sort(
        sorted_works, [](const auto& left, const auto& right) {
            return id_less(
    ...
    plan["plan_id"] =
        "candidate_plan_" + crypto::sha256(canonical_without_id).substr(0, 32);
    return plan;
}
    ...
    nlohmann::ordered_json contract {
        ...
        { "algorithm_version",
          materialization.at("algorithm").at("name").get<std::string>() + "-"
              + materialization.at("algorithm").at("version").get<std::string>() },
        { "configuration",
...
nlohmann::ordered_json candidate_planner::enrichment_fetch_plan(
    ...
    std::string source_name, std::string locator, std::string created_at
) {
