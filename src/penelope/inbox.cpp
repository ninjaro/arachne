...
public:
    statement(sqlite3* const database, const std::string_view sql)
        ...
        if (sqlite3_prepare_v2(
            ...
            != SQLITE_OK) {
            throw database_error(sqlite_message(database, "prepare statement"));
        }
    }
    ...
    void bind_json_value(const int index, const json& value) {
        if (value.is_null()) {
            ...
            bind(index, static_cast<std::int64_t>(value.get<bool>() ? 1 : 0));
    ...
    [[nodiscard]] bool step() {
        ...
        throw database_error(sqlite_message(database_, "execute statement"));
    }
...
public:
    database(
        ...
        if (sqlite3_open_v2(native.c_str(), &value_, flags, nullptr)
            != SQLITE_OK) {
            const std::string message = sqlite_message(value_, "open database");
            if (value_ != nullptr) {
...
void validate_integer_or_local_reference_shape(
    ...
    if (found->is_string()) {
        ...
        add_issue(
            batch, "type_mismatch", path + "/" + pointer_escape(key),
            "reference must be a positive database integer or local_id", &*found
        );
    } else {
        try {
...
void validate_create_financial(
    ...
    if (value.contains("amount_min") && value["amount_min"].is_number_integer()
    ...
    if (value.contains("currency_code") && value["currency_code"].is_string()) {
...
void validate_evidence_list(
    ...
    if (found->empty()) {
        add_issue(
            ...
            "semantic assertion needs at least one evidence reference", &*found
        );
    }
            ...
            try {
                if (reference.get<std::int64_t>() <= 0) {
                    add_issue(
                        batch, "number_out_of_range",
                        indexed_path(path + "/evidence", index),
                        "evidence row reference must be positive", &reference
                    );
                }
            } catch (const json::exception&) {
                add_issue(
...
void validate_update_record(
    ...
    if (integer_id) {
        ...
        if (id != value.end() && id->is_string()
            && !valid_canonical_id(id->get_ref<const std::string&>(), family)) {
            add_issue(
        ...
        for (const auto& [field, field_value] : set->items()) {
            ...
            if (expected == mutable_fields.end()) {
                add_issue(
                    ...
                    "field is not mutable for this entity family", &field_value
                );
                continue;
            }
        ...
        if (table == "ingest_issues") {
            ...
            for (std::size_t index = 0; index < rows->size(); ++index) {
                ...
                if (const auto json_path = value.find("json_path");
                    ...
                    && !json_path->get_ref<const std::string&>().starts_with(
                        '/'
                    )) {
                    add_issue(
...
void validate_merge_record(
    ...
    if (members != value.end() && members->is_array()) {
        ...
        for (std::size_t index = 0; index < members->size(); ++index) {
            ...
            if (!member.is_string()
                || !valid_canonical_id(
                    member.is_string() ? member.get_ref<const std::string&>()
                                       : std::string_view(),
                    family
                )) {
                add_issue(
...
[[nodiscard]] json parse_strict_json(const std::string& bytes) {
    ...
    try {
        ...
        throw inbox_error(std::string("invalid UTF-8 JSON: ") + error.what());
    }
}
...
[[nodiscard]] bool row_exists(
    ...
    const std::string sql = "SELECT 1 FROM " + std::string(table) + " WHERE "
...
void prevalidate_semantics(parsed_batch& batch, database& product) {
    ...
    auto walk = [&batch,
        ...
        for (std::size_t index = 0; index < found->size(); ++index) {
            callback((*found)[index], indexed_path("/create/" + key, index));
        }
    };
    ...
    auto assertion_evidence = [&](const json& row, const std::string& path) {
        ...
        for (std::size_t index = 0; index < values->size(); ++index) {
            ...
            prevalidate_integer_or_local_reference(
                ...
                indexed_path(path + "/evidence", index), "evidence", "evidence"
            );
        }
    };
    ...
    walk("concept_relations", [&](const json& row, const std::string& path) {
        prevalidate_entity_reference(
    ...
    if (source_updates != update.end() && source_updates->is_array()) {
        for (std::size_t index = 0; index < source_updates->size(); ++index) {
            ...
            if (table == "ingest_issues") {
                ...
                for (std::size_t index = 0; index < values.size(); ++index) {
                    ...
                    if (!issue.step()) {
                        ...
                        add_issue(
                            batch, "open_issue_delete_forbidden", value_path,
            ...
            for (const auto& [assertion_table, link_table] :
                ...
                for (const auto assertion_id : assertions) {
                    ...
                    while (remaining.step()) {
                        if (!deleted_evidence.contains(remaining.integer(0))) {
                            has_remaining = true;
                            break;
                        }
                    }
...
void resolve_scalar_fields(
    ...
    for (const auto& field : columns) {
        ...
        auto inspect = [&](const std::string& id, const bool is_target) {
            statement query(
                sql,
                "SELECT " + sql_column(field) + " FROM " + std::string(table)
...
void merge_concept_relations(
    ...
    for (const auto& row : rows) {
        ...
        const std::string object = row.object == member ? target : row.object;
        if (subject == object) {
            statement remove(sql, "DELETE FROM concept_relations WHERE id=?");
            remove.bind(1, row.id);
            remove.execute();
            continue;
        }
...
[[nodiscard]] std::vector<std::string> merge_members(const json& operation) {
...
void ensure_product_database(
    ...
    try {
        ...
        if (sqlite3_exec(
            ...
            sqlite3_free(error);
            throw inbox_error("cannot initialize product database: " + message);
        }
        sqlite3_close(raw);
    } catch (...) {
        sqlite3_close(raw);
...
[[nodiscard]] std::vector<file_snapshot> snapshot_inbox(const fs::path& inbox) {
    ...
    for (fs::directory_iterator iterator(inbox, error), end;
        ...
        struct stat state {};
        if (::lstat(path.c_str(), &state) != 0) {
            throw inbox_error("cannot inspect inbox entry: " + path.string());
        }
        ...
        if (path.extension() != ".json") {
            throw inbox_error(
                "only plain .json batch files are accepted: " + path.string()
            );
        }
...
void reject_duplicate_pending_ids(
    ...
    std::map<std::string, std::vector<std::size_t>, std::less<>> occurrences;
...
inbox_result check_product_inbox(const fs::path& repository_root) {
    return run_inbox(repository_root, false);
}
