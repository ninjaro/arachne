...
namespace {
    ...
    public:
        ...
        void execute(const std::string_view sql) {
            char* error = nullptr;
            if (sqlite3_exec(value_, sql.data(), nullptr, nullptr, &error)
                != SQLITE_OK) {
    ...
    [[nodiscard]] fs::path product_path(const fs::path& state_root) {
        return state_root / "database" / "art-islands.sqlite";
    }

    [[nodiscard]] bool ensure_real_store_directory(
        const fs::path& repository_root, const bool create
    ) {
        for (const auto& component : { repository_root / ".arachne",
    ...
    void attach_product_read_only(database& hints, const fs::path& path) {
        ...
        // Preparing the fields consumed by this commit is a strict
        // current-shape check. Numeric application schema versions and
        // historical dispatch are intentionally not part of the product
        // contract.
    ...
    void initialize_store(
        ...
        hints.execute(
            ...
            "analytical_observations(left_family,left_id,right_family,right_id,"
            "id);"
            ...
            " right_id,json_extract(extra_json,'$.right_channel'),metric,scope,"
            "id)"
    ...
    [[nodiscard]] std::string
    metadata_value(sqlite3* const sql, const std::string_view key) {
    ...
    void require_current_store(
        ...
        if (!version.step()
            || version.integer(0) != hint_store_schema_version) {
            throw merge_hint_store_error(
                "unsupported disposable merge-hint schema version"
            );
        }
        if (metadata_value(hints.native(), "hint_store_schema_version")
            != "3") {
            throw merge_hint_store_error(
    ...
    [[nodiscard]] json
    optional_integer(const statement& query, const int column) {
    ...
    [[nodiscard]] ignored_pair_state load_ignored_pairs(const fs::path& path) {
        ...
        if (!document.is_object()
            ...
            || document.value("format_version", 0) != 1 || document.size() != 3U
        ...
        for (const auto& value : document.at("ignored_pairs")) {
            if (!value.is_object() || value.size() != 3U
                ...
                || !value.contains("left_id")
                || !value.at("left_id").is_string()
    ...
    [[nodiscard]] json build_input(database& hints) {
        ...
        while (base.step()) {
            ...
            if (base.text(1) == "agent") {
                ...
                entity["work"] = {
                    { "medium",
                      base.is_null(5) ? json(nullptr) : json(base.text(5)) },
                    { "year_start", optional_integer(base, 6) },
                    { "year_end", optional_integer(base, 7) },
                    { "date_precision",
                      base.is_null(8) ? json(nullptr) : json(base.text(8)) },
                    { "date_start_text",
                      base.is_null(9) ? json(nullptr) : json(base.text(9)) },
                    { "date_end_text",
                      base.is_null(10) ? json(nullptr) : json(base.text(10)) },
                    { "date_qualifier",
                      base.is_null(11) ? json(nullptr) : json(base.text(11)) },
                    { "credits", json::array() },
        ...
        statement manifestations(
            hints.native(),
            "SELECT "
            "entity_id,work_id,manifestation_type,release_year,region_code,"
        ...
        while (manifestations.step()) {
            ...
            json value {
                ...
                { "region_code",
                  manifestations.is_null(4) ? json(nullptr)
                                            : json(manifestations.text(4)) },
                { "language_code",
                  manifestations.is_null(5) ? json(nullptr)
                                            : json(manifestations.text(5)) },
                { "label",
                  manifestations.is_null(6) ? json(nullptr)
        ...
        while (memberships.step()) {
            ...
            const auto append = [&](const auto found,
                ...
                json value {
                    ...
                    { "position_text",
                      memberships.is_null(4) ? json(nullptr)
        ...
        while (agent_relations_query.step()) {
            ...
            const auto append = [&](const auto found,
                ...
                json value {
                    ...
                    { "period_text",
                      agent_relations_query.is_null(5)
                          ? json(nullptr)
                          : json(agent_relations_query.text(5)) },
                    { "role_text",
                      agent_relations_query.is_null(6)
                          ? json(nullptr)
        ...
        statement events(
            ...
            " date_precision,place_text FROM product.events ORDER BY "
            "entity_id,id"
        );
        while (events.step()) {
            json value {
                ...
                { "date_text",
                  events.is_null(5) ? json(nullptr) : json(events.text(5)) },
                { "date_precision",
                  events.is_null(6) ? json(nullptr) : json(events.text(6)) },
                { "place_text",
                  events.is_null(7) ? json(nullptr) : json(events.text(7)) },
            };
            ...
            if (found != indices.end()
                ...
                entities[found->second]["work"]["measurements"].push_back(
                    { { "type", measurements.text(1) },
                      ...
                      { "qualifier",
                        measurements.is_null(4) ? json(nullptr)
        ...
        while (evidence.step()) {
            ...
            const auto key = std::tuple { assertion_id, evidence.integer(1),
                                          evidence.integer(2) };
        ...
        std::map<std::int64_t, std::vector<assertion_location>>
            relation_locations;
        statement relations(
            hints.native(),
        ...
        while (relations.step()) {
            ...
            json common {
                ...
                { "region_code",
                  relations.is_null(7) ? json(nullptr)
                                       : json(relations.text(7)) },
                { "confidence",
                  relations.is_null(8) ? json(nullptr)
        ...
        while (relation_evidence.step()) {
            ...
            for (const auto& location : locations->second) {
                auto& relation = entities[location.first]["concept"]
                                         ["neighbors"][location.second];
    ...
    using canonical_family_index = std::unordered_map<std::string, std::string>;
    ...
    [[nodiscard]] canonical_family_index
    canonical_entity_families(sqlite3* const sql) {
    ...
    [[nodiscard]] canonical_type_index
    canonical_entity_types(sqlite3* const sql) {
        ...
        while (entities.step()) {
            result.emplace(
                ...
                canonical_type_descriptor {
                    ...
                    .family_type
                    = entities.is_null(2) ? "unknown" : entities.text(2),
                }
            );
        }
        return result;
    ...
    [[nodiscard]] canonical_work_date_index
    canonical_work_dates(sqlite3* const sql) {
        canonical_work_date_index result;
        statement works(
            sql,
            "SELECT "
            "entity_id,year_start,year_end,date_precision,date_start_text,"
        ...
        while (works.step()) {
            result.emplace(
                ...
                json {
                    ...
                    { "date_precision",
                      works.is_null(3) ? json("unknown")
                                       : json(works.text(3)) },
                    { "date_start_text",
                      works.is_null(4) ? json(nullptr) : json(works.text(4)) },
                    { "date_end_text",
                      works.is_null(5) ? json(nullptr) : json(works.text(5)) },
                    { "date_qualifier",
                      works.is_null(6) ? json(nullptr) : json(works.text(6)) },
                }
            );
        }
        return result;
    ...
    [[nodiscard]] canonical_concept_relation_index
    canonical_concept_relations(sqlite3* const sql) {
    ...
    void validate_entity_id_array(
        ...
        for (std::size_t index = 0; index < values.size(); ++index) {
            ...
            if (!value.is_string()
                ...
                invalid_projection(
                    item_context + " must be a non-empty string"
                );
            }
            require_canonical_entity(
                entities, value.get_ref<const std::string&>(), family,
                item_context
            );
        }
    }
    ...
    void validate_temporal_bucket_references(
        ...
        for (std::size_t date_index = 0; date_index < dates.size();
             ++date_index) {
            const std::string date_context = std::string(context)
                + ".date_values[" + std::to_string(date_index) + "]";
            require_canonical_entity(
                entities,
        ...
        if (dated_work_ids != bucket_work_ids
            || dates.size() != dated_work_ids.size()) {
            invalid_projection(
                std::string(context)
        ...
        for (std::size_t concept_index = 0; concept_index < concepts.size();
             ++concept_index) {
            const std::string concept_context = std::string(context)
                + ".concepts[" + std::to_string(concept_index) + "]";
            require_canonical_entity(
                entities,
                required_string(
    ...
    void validate_observation_detail_references(
        ...
        for (std::size_t index = 0; index < observations.size(); ++index) {
            ...
            const json& details
                = required_object(observation, "details", observation_context);
            ...
            if (details.contains("explicit_concept_relations")) {
                const json& relations
                    = details.at("explicit_concept_relations");
                ...
                for (std::size_t relation_index = 0;
                    ...
                    validate_internal_id_array(
                        evidence_ids,
                        required_array(
                            relation, "evidence_ids", relation_context
                        ),
                    ...
                    validate_internal_id_array(
                        source_ids,
                        required_array(
                            relation, "source_ids", relation_context
                        ),
            ...
            if (details.contains("explicit_concept_relation_ids")) {
                ...
                if (!relation_ids.is_array()) {
                    invalid_projection(
                        context
                        + ".explicit_concept_relation_ids must be an array"
                    );
                }
                for (std::size_t relation_index = 0;
                    ...
                    if (!id.is_number_integer() && !id.is_number_unsigned()) {
                        invalid_projection(
                            context
                            + ".explicit_concept_relation_ids contains "
                              "a non-integer"
                        );
                    }
            ...
            if (details.contains("bridge_works")) {
                ...
                if (!rows.is_array()) {
                    invalid_projection(
                        context + ".bridge_works must be an array"
                    );
                }
                for (std::size_t row_index = 0; row_index < rows.size();
                    ...
                    for (const auto* field :
                        ...
                        if (row.contains(field)) {
                            require_canonical_entity(
                                entities,
                                required_string(row, field, row_context),
            ...
            if (details.contains("gaps")) {
                ...
                for (std::size_t gap_index = 0; gap_index < gaps.size();
                    ...
                    const std::string gap_context
                        = context + ".gaps[" + std::to_string(gap_index) + "]";
                    if (!gap.is_object()) {
    ...
    void validate_fingerprint_references(
        ...
        for (std::size_t index = 0; index < fingerprints.size(); ++index) {
            ...
            for (const auto& [work_id, weight] :
                 required_object(fingerprint, "work_distribution", context)
                     .items()) {
            ...
            if (exact_dates != expected_exact_dates) {
                invalid_projection(
                    context
                    + ".exact_canonical_work_dates does not completely "
                      "preserve "
    ...
    void validate_concept_clustering_references(
        ...
        for (std::size_t index = 0; index < clusterings.size(); ++index) {
            const std::string clustering_context = std::string(context_prefix)
                + "[" + std::to_string(index) + "]";
            ...
            for (std::size_t cluster_index = 0; cluster_index < clusters.size();
                ...
                const json& members = required_array(
                    clusters.at(cluster_index), "members", context
                );
                for (std::size_t member_index = 0;
                    ...
                    require_canonical_entity(
                        entities,
                        required_string(
                            members.at(member_index), "concept_id",
                            member_context
                        ),
    ...
    void validate_medium_chronology_references(
        ...
        const std::string_view context,
        const std::string_view first_concept_field,
        ...
        for (std::size_t index = 0; index < summaries.size(); ++index) {
            ...
            validate_entity_id_array(
                entities,
                required_array(summary, "concept_ids", summary_context),
            ...
            for (std::size_t example_index = 0; example_index < examples.size();
                ...
                const std::string example_context = summary_context
                    + ".examples[" + std::to_string(example_index) + "]";
                for (const auto field :
                     { first_concept_field, second_concept_field }) {
                    require_canonical_entity(
                        entities,
                        required_string(example, field, example_context),
                        "concept", example_context + "." + std::string(field)
                    );
                }
            }
        }
    ...
    void validate_undominated_cross_media_cluster_references(
        ...
        for (std::size_t index = 0; index < clusters.size(); ++index) {
            ...
            validate_entity_id_array(
                entities,
                required_array(cluster, "concept_ids", cluster_context),
            ...
            for (std::size_t channel_index = 0; channel_index < channels.size();
                 ++channel_index) {
                const std::string channel_context = cluster_context
                    + ".channels[" + std::to_string(channel_index) + "]";
                require_canonical_entity(
                    entities,
                    required_string(
                        channels.at(channel_index), "concept_id",
                        channel_context
                    ),
            ...
            require_canonical_entity(
                entities,
                required_string(
                    cluster, "dominant_concept_id", cluster_context
                ),
    ...
    void validate_cross_media_references(
        ...
        for (std::size_t index = 0; index < profiles.size(); ++index) {
            ...
            require_canonical_entity(
                entities,
                required_string(profile, "concept_id", profile_context),
                "concept", profile_context + ".concept_id"
            );
            const json& media
                = required_array(profile, "media", profile_context);
            for (std::size_t medium_index = 0; medium_index < media.size();
                ...
                validate_entity_id_array(
                    entities,
                    required_array(channel, "work_ids", channel_context),
                    "work", channel_context + ".work_ids"
                );
                validate_entity_id_array(
                    entities,
                    required_array(channel, "agent_ids", channel_context),
        ...
        for (std::size_t index = 0; index < same_concept.size(); ++index) {
            ...
            require_canonical_entity(
                entities,
                required_string(
                    same_concept.at(index), "concept_id", row_context
                ),
        ...
        validate_medium_chronology_references(
            entities,
            required_array(
                cross_media, "synchronized_medium_summaries", context
            ),
        ...
        for (std::size_t index = 0; index < disagreements.size(); ++index) {
            ...
            for (std::size_t boundary_index = 0;
                ...
                require_canonical_entity(
                    entities,
                    required_string(
                        boundaries.at(boundary_index), "concept_id",
                        boundary_context
                    ),
        ...
        for (std::size_t index = 0; index < bridge_agents.size(); ++index) {
            ...
            require_canonical_entity(
                entities,
                required_string(
                    bridge_agents.at(index), "agent_id", row_context
                ),
    ...
    void validate_centrality_diagnostic_references(
        ...
        for (const auto* field :
             { "overall", "by_concept_type", "by_relation_type", "by_medium",
               "confidence_presence", "historical_role_distribution" }) {
        ...
        for (std::size_t index = 0; index < scale_coverage.size(); ++index) {
            ...
            require_canonical_entity(
                entities,
                required_string(
                    scale_coverage.at(index), "work_id", row_context
                ),
                "work", row_context + ".work_id"
            );
        }
        ...
        for (std::size_t index = 0; index < saturation.size(); ++index) {
            ...
            require_canonical_entity(
                entities,
                required_string(
                    saturation.at(index), "concept_id", row_context
                ),
                "concept", row_context + ".concept_id"
            );
        }
        ...
        for (const auto* field :
             { "material_examples", "negligible_examples" }) {
        ...
        for (std::size_t index = 0; index < clusterings.size(); ++index) {
            ...
            for (std::size_t cluster_index = 0; cluster_index < clusters.size();
                ...
                for (std::size_t member_index = 0;
                     member_index < members.size(); ++member_index) {
                    validate_typed_entity_reference(
                        entities, members.at(member_index), "entity_id",
                        "family",
    ...
    void validate_priority_detail_references(
        ...
        for (const auto* field : { "contributions", "bridge_works" }) {
            ...
            for (std::size_t index = 0; index < rows.size(); ++index) {
                ...
                const std::string row_context = std::string(context)
                    + ".details." + field + "[" + std::to_string(index) + "]";
                require_canonical_entity(
                    entities, required_string(row, "work_id", row_context),
                    "work", row_context + ".work_id"
                );
    ...
    void validate_priority_references(
        ...
        for (std::size_t index = 0; index < priorities.size(); ++index) {
            ...
            const std::string context
                = "analysis.research_priorities[" + std::to_string(index) + "]";
            const auto& family
            ...
            if (family == "agent" || family == "work" || family == "concept") {
                ...
                if (separator == std::string::npos || separator == 0U
                    ...
                    invalid_projection(
                        context
                        + ".entity_id must identify a canonical concept pair"
                    );
                }
                require_canonical_entity(
                    entities, std::string_view(id).substr(0U, separator),
                    "concept", context + ".entity_id.left"
                );
                require_canonical_entity(
                    entities, std::string_view(id).substr(separator + 1U),
                    "concept", context + ".entity_id.right"
                );
            } else {
                invalid_projection(
                    context
                    + ".entity_family must be agent, work, concept, or "
                      "concept_pair"
                );
            }
    ...
    void validate_trajectory_references(
        const canonical_family_index& entities,
        const canonical_type_index& types, const json& signatures
    ) {
        for (std::size_t index = 0; index < signatures.size(); ++index) {
    ...
    void validate_pair_view_rows(
        ...
        for (std::size_t index = 0; index < rows.size(); ++index) {
            ...
            validate_typed_entity_reference(
                entities, rows.at(index), "right_id", "right_family",
                row_context
            );
        }
    }
    ...
    void validate_view_references(
        const canonical_family_index& entities,
        const canonical_type_index& types, const json& views
    ) {
        ...
        for (const auto& [metric, groups] : top_neighbors.items()) {
            if (!groups.is_array()) {
                invalid_projection(
                    "analysis.views.top_neighbors." + metric
                    + " must be an array"
                );
            }
            for (std::size_t index = 0; index < groups.size(); ++index) {
                const json& group = groups.at(index);
                const std::string context = "analysis.views.top_neighbors."
                    + metric + "[" + std::to_string(index) + "]";
                validate_typed_entity_reference(
                ...
                for (std::size_t neighbor_index = 0;
                    ...
                    validate_typed_entity_reference(
                        ...
                        "neighbor_family",
                        context + ".neighbors[" + std::to_string(neighbor_index)
                            + "]"
                    );
                }
            }
        ...
        for (std::size_t index = 0; index < bridge_concepts.size(); ++index) {
            ...
            validate_entity_id_array(
                entities, required_array(row, "neighbor_ids", context),
                "concept", context + ".neighbor_ids"
            );
        }
        const json& bridge_works
        ...
        for (std::size_t index = 0; index < bridge_works.size(); ++index) {
            const json& row = bridge_works.at(index);
            const std::string context
                = "analysis.views.bridge_works[" + std::to_string(index) + "]";
            require_canonical_entity(
        ...
        validate_trajectory_references(
            entities, types,
            required_array(
                views, "sequence_alignment_outliers", "analysis.views"
            )
        );
    }
    ...
    void validate_ancestry_references(
        ...
        const json& edges = required_array(
            chronological, "edges", "analysis.ancestry.chronological"
        );
        ...
        const json& views
            = required_object(ancestry, "views", "analysis.ancestry");
        for (const auto* field :
             { "similar_entities_with_little_or_no_shared_ancestry",
               "cross_branch_structural_convergence" }) {
            const json& rows
                = required_array(views, field, "analysis.ancestry.views");
            for (std::size_t index = 0; index < rows.size(); ++index) {
                validate_ancestry_comparison(
    ...
    void validate_external_classification_comparison(
        const canonical_family_index& entities,
        const canonical_type_index& types, const json& comparison,
        const std::string_view algorithm_version, const json& snapshot
    ) {
        ...
        if (required_boolean(comparison, "treated_as_ground_truth", context)
            ...
            invalid_projection(
                context + " violates the analytical-only policy"
            );
        }
        ...
        const double containment_threshold
        ...
        for (std::size_t index = 0; index < rows.size(); ++index) {
            ...
            require_canonical_entity(
                entities, broader, "concept",
                row_context + ".broader_concept_id"
            );
            require_canonical_entity(
            ...
            if (!provider_relation_id.is_null()
                && (!provider_relation_id.is_string()
                    || provider_relation_id.get_ref<const std::string&>()
                           .empty())) {
                invalid_projection(
            ...
            if (shared_support > broader_support
                ...
                invalid_projection(
                    row_context + " has impossible work support"
                );
            }
            ...
            const double broader_inside_narrower
                = required_finite_number(
                      ...
                      .get<double>();
            const double margin
                = required_finite_number(row, "directional_margin", row_context)
                      .get<double>();
            ...
            if (narrower_inside_broader < 0.0 || narrower_inside_broader > 1.0
                ...
                || margin > 1.0
                || std::abs(narrower_inside_broader - expected_forward)
                    > tolerance
                || std::abs(broader_inside_narrower - expected_reverse)
                    > tolerance
                || std::abs(margin - (expected_forward - expected_reverse))
                    > tolerance) {
                invalid_projection(
                    row_context + " has inconsistent measurements"
                );
            }
        ...
        for (const auto& [classification, count] : actual_counts) {
            if (required_integer(
                ...
                != count) {
                invalid_projection(
                    context + ".summary counts are inconsistent"
                );
            }
        }
        const std::string& status
            = required_string(comparison, "status", context);
        const json& input = required_field(comparison, "input", context);
        if (status == "not_supplied") {
            if (!input.is_null() || !rows.empty()) {
                invalid_projection(
                    context
                    + " not_supplied output must contain no input or rows"
                );
            }
            ...
            if (!input.is_object()
                ...
                || required_integer(input, "version", context + ".input", 1, 1)
                    != 1) {
                invalid_projection(context + ".input has an invalid contract");
            }
            static_cast<void>(
                required_string(input, "provider", context + ".input")
            );
            static_cast<void>(
                required_string(input, "dataset_version", context + ".input")
            );
            if (required_integer(
                ...
                invalid_projection(
                    context + ".input relation count is invalid"
                );
            }
        } else {
    ...
    void validate_external_classification_manifest(
        ...
        if (required_string(summary, "status", context) != status
            ...
            || required_boolean(summary, "used_by_this_run", context)
                != supplied
            ...
            || required_boolean(
                summary, "used_to_calibrate_parameters", context
            )
            || required_boolean(
    ...
    void validate_analysis_entity_references(
        ...
        validate_priority_references(
            entities, analysis.at("research_priorities")
        );
    ...
    [[nodiscard]] json stored_analysis_section(
        ...
        if (result.is_discarded()) {
            throw merge_hint_store_error(
                "structural analysis projection contains invalid JSON in "
                "section "
                + std::string(section)
            );
        }
    ...
    void validate_stored_extended_analysis_references(sqlite3* const sql) {
        ...
        if (!cross_media.is_object() || !centrality.is_object()
            ...
            || !mixed.is_object() || !external.is_object()) {
            throw merge_hint_store_error(
    ...
    void validate_analysis_for_storage(const json& projection) {
        const json& analysis
            = required_field(projection, "analysis", "projection");
        ...
        if (analysis_algorithm_version
            != arachnespace::contracts::structural_analysis_algorithm_version) {
            invalid_projection(
                "analysis.algorithm_version does not match the pinned "
                "structural "
                "algorithm version"
            );
        }
        ...
        for (const auto* section :
             { "work_quality", "sequences", "trajectory_signatures",
               "clusterings", "structural_fingerprints",
               "genre_like_signatures", "research_priorities" }) {
        ...
        for (std::size_t index = 0; index < observations.size(); ++index) {
            ...
            static_cast<void>(
                required_string(value, "left_entity_type", context)
            );
            static_cast<void>(
                required_string(value, "right_entity_type", context)
            );
            static_cast<void>(
                required_string(value, "left_family_type", context)
            );
            static_cast<void>(
            ...
            if (left == right) {
                ...
                const bool valid_channels = left_family == "concept"
                    && right_family == "concept" && left_channel != value.end()
                    && left_channel->is_string()
                    && !left_channel->get_ref<const std::string&>().empty()
                    && right_channel != value.end()
                    && right_channel->is_string()
    ...
    void validate_projection_for_storage(const json& projection) {
        ...
        for (std::size_t index = 0; index < projection.at("blocks").size();
             ++index) {
        ...
        for (std::size_t index = 0; index < projection.at("memberships").size();
             ++index) {
        ...
        for (std::size_t index = 0; index < projection.at("candidates").size();
             ++index) {
            ...
            for (const auto field :
                ...
                static_cast<void>(
                    required_integer(value, field, context, 0, 10'000)
                );
            }
            static_cast<void>(
                required_boolean(value, "strong_identity", context)
            );
            ...
            const json& component
                = required_field(value, "component_id", context);
        ...
        for (std::size_t index = 0;
            ...
            if (!statistic_families.emplace(family).second) {
                invalid_projection(
                    "family statistics contain a duplicate family"
                );
            }
            static_cast<void>(required_integer(
            ...
            for (const auto field :
                 { "candidate_count", "strong_identity_count",
                   "selected_count" }) {
                static_cast<void>(required_integer(
        ...
        if (statistic_families
            != std::set<std::string, std::less<>> { "agent", "work",
                                                    "concept" }) {
            invalid_projection(
                "family statistics must cover all three families"
            );
        }
    ...
    void require_derived_consistency(sqlite3* const sql) {
        ...
        require_zero(
            ...
            "merge-hint state references an unknown or mismatched canonical "
            "entity"
        );
        require_zero(
            ...
            " WHERE p.section='structural_fingerprints' AND (e.id IS NULL OR "
            "NOT("
        ...
        require_zero(
            ...
            "analytical observations are stale or unbound from the product "
            "snapshot"
        );
        require_zero(
            "SELECT count(*) FROM ("
        ...
        require_zero(
            ...
            "  'arachne_structural_analysis_v1')"
            " OR (section='version' AND json_extract(payload_json,'$') IS NOT "
            "1)"
            ...
            "   (SELECT value FROM main.metadata WHERE key='product_sha256')))",
            "structural analysis projection has stale contract or snapshot "
            "binding"
        );
        require_zero(
            "SELECT count(*) FROM main.analytical_observations o WHERE"
            " o.algorithm_version IS NOT (SELECT json_extract(payload_json,'$')"
            " FROM main.analysis_projections WHERE "
            "section='algorithm_version')",
            "analytical observations do not match the structural algorithm "
            "version"
        );
        require_zero(
            "SELECT count(*) FROM main.analysis_projections WHERE"
            " section='algorithm_version' AND json_extract(payload_json,'$') "
            "IS NOT"
            " (SELECT value FROM main.metadata"
            "  WHERE key='structural_algorithm_version')",
...
fs::path merge_hint_decisions_path(const fs::path& state_root) {
    return state_root / "database" / "merge-hint-decisions.json";
}
...
json prepare_merge_hint_rebuild(
    ...
    const fs::path product = product_path(state_root);
    ...
    const ignored_pair_state decisions
        = load_ignored_pairs(merge_hint_decisions_path(state_root));
    const fs::path store = merge_hint_store_path(source_root);
    static_cast<void>(ensure_real_store_directory(source_root, true));
    ...
    try {
        ...
        initialize_store(hints, product_sha256, generator_version, decisions);
...
void store_merge_hint_projection(
    ...
    const fs::path store = merge_hint_store_path(source_root);
    const fs::path product = product_path(state_root);
    if (!ensure_real_store_directory(source_root, false)
    ...
    require_current_store(
        hints, product, merge_hint_decisions_path(state_root)
    );
    if (!projection.is_object()
        || projection.value("artifact_type", "") != "merge_hint_projection_v1"
        ...
        || projection.at("decisions_snapshot").value("sha256", "")
            != metadata_value(hints.native(), "decisions_sha256")
        || projection.at("decisions_snapshot").value("ignored_pair_count", -1)
            != std::stoll(
                metadata_value(hints.native(), "ignored_pair_count")
            )) {
        throw merge_hint_store_error(
    ...
    for (const auto& value : projection.at("memberships")) {
        membership.reset();
        membership.bind(
            1,
            required_integer(
        ...
        membership.bind(2, required_string(value, "entity_id", "membership"));
        membership.execute();
    }
    ...
    statement candidate(
        ...
        "graph_basis_points,context_basis_points,strong_identity,ignored,"
        "selected,"
    ...
    for (const auto& value : projection.at("candidates")) {
        ...
        candidate.bind(
            5,
            required_integer(value, "text_basis_points", "candidate", 0, 10'000)
        );
        candidate.bind(
            6,
            required_integer(
        ...
        candidate.bind(12, value.at("supports").dump());
        candidate.bind(13, value.at("signals").dump());
        candidate.bind(14, value.at("selection_reasons").dump());
        candidate.execute();
    }
    ...
    for (const auto& value : projection.at("family_statistics")) {
        statistic.reset();
        statistic.bind(1, required_string(value, "family", "statistic"));
        statistic.bind(
            2,
            required_integer(
                value, "adaptive_threshold_basis_points", "statistic", 0, 10'001
            )
        );
        statistic.bind(
            3,
    ...
    while (candidates.step()) {
        projection["candidates"].push_back(
            { { "family", candidates.text(0) },
              ...
              { "selected", true },
              { "component_id",
                candidates.is_null(8) ? json(nullptr)
