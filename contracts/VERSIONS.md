...
`arachne_batch` is the one product-inbox format in the current commit. It is a
direct, transactional request for human-owned taxonomy, evidence, and
assertions rather than a provider refresh, generic actor message, or universal
normalized transfer manifest.
...
|---|---|---|---|
| `arachne_batch` | Research bot or reviewer | Arachne product inbox | Closed human-knowledge request. It admits concepts and concept names, human sources/evidence/assertions, concept merges, and compare-and-swap provider-ID corrections. Assertions require explicit source-backed evidence, exact quotes, and stances. |
...
- `external_enrichment_review_v1` is a disposable, snapshot-bound comparison
  ...
  disagreements, media-link suggestions, and raw unmapped values. It has no
  canonical write authority. General information is accepted only through the
  provider materializer; human semantic changes still use `arachne_batch`.
Legacy normalized imports and unresolved JSONL formats are not active contracts.
Routine inbox processing mutates only the human-owned portion of the current
product schema, plus the narrow conditional replacement of a wrong provider
ID. Its pair-local work-concept centrality scale is explicit: new assignments
require a reviewed `binary`, `ordinal`, or `graded` mode, while `none` remains
reserved for not-yet-reviewed assignments already present in the canonical
...
- Removing a field, changing a field's meaning or type, broadening execution
  authority, or changing hash semantics requires a new major actor contract.
  `arachne_batch` has no version negotiation: only the shape in the selected
  repository commit is supported.
